...
int findRoomIndex(String roomName) {
  for (int i = 0; i < mainMenuCount; i++) {
    if (mainMenu[i].startsWith(roomName + "-") || mainMenu[i] == roomName) {
      return i;
    }
  }
  return -1;
...
void addRoom(String roomName) {
  ...
  mainMenu[mainMenuCount++] = roomName; // no devices yet
...
void addDevice(String roomName, String deviceName) {
  ...
  if (mainMenu[idx].indexOf('-') == -1) {
    // first device
    mainMenu[idx] += "-" + deviceName;
  } else {
    // append with comma
    mainMenu[idx] += "," + deviceName;
  }
